...
<p>
    ...
    <a href="/docs/track-order.md">
     ...
     <strong>Accept Order for Processing</strong>
</p>

### Accept Order for Processing

| Use Case Name | Accept Order for Processing |
| ------------ | ----- |    
| Summary: | The vendor accepts the order request from the customer to process. |
| Actors: | Vendor |
| Preconditions: | An order request has been made to the vendor. |
| Postconditions: | The order is accepted by the vendor, and is classified as a "Running Order" by the system.
| Basic Flow | **Actor Action** |
|            | 1. Vendor navigates to the order page using the "Order" tab. |
|            | 2. Vendor clicks the actions icon. |
|            | 3. Vendor selects "Process Order" |               
|            | **System Action** | 
|            | 1.1 The system navigates to the order page.|
|            | 2.1 The system displays the action buttons modal.|
|            | 3.1 The system classifies the order as a "Running Order".|
|           | |
